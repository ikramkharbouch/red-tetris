...
export function useBoard() {
    ...
    // put the tetromino into the stage tetrominoes and merge
    const [display, setDisplay] = useState( ()=> mergeIntoStage(scene, shape, position) );
    // the score of every line that is finished
    ...
    // moving the position of the tetro using key directions
    function tick() {
        if (!movePosition(0, 1)) {
            placeShape();
        }
